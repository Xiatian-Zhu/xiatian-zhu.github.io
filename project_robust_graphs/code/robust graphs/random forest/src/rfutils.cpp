...
{
    int i, j, k;
	// Synthesize non-existing data points by independently samply each variable with a uniform distribution
    for (i = realN; i < totalN; ++i) 
	{
		...
		// Note Y_nv_src_xx and x are with different metrix directions
		
		// Deal with discrete non-visual data
        for (int idn_nv = 0; idn_nv < num_nv_src_discrete; ++idn_nv)
        {
            k = (int) (unif_rand() * realN);
            Y_nv_src_discrete[idn_nv * totalN+(i)] = Y_nv_src_discrete[idn_nv * totalN + (k)];
        }
		
		// Deal with continuous non-visual data
        for (int idn_nv = 0; idn_nv < num_nv_src_continuous; ++idn_nv)
        {
            k = (int) (unif_rand() * realN);
            Y_nv_src_continuous[idn_nv * totalN+(i)] = Y_nv_src_continuous[idn_nv * totalN + (k)];
        }
    }
}
...
/* makeA() constructs the mdim by nsample integer array a.  For each 
   ...
   - $b$: record the order of sample given a variable, '1' refers to the minimun.
	e.g. four 1-d samples with value as 2.4 9.0 -1 2.4, then b[0-2] = {2 3 1 2};
   */
void makeA(double *x, const int mdim, const int nsample, int *cat, int *a, int *b) 
{
	// printf("To make matrax a & b ----");
    int i, j, n1, n2;
    ...
    for (i = 0; i < mdim; ++i) 
	{
		...
		{
			// The index of samples, the original order of data points, starting from 1
            for (j = 0; j < nsample; ++j) 
			{
                v[j] = x[i + j * mdim];
                index[j] = j + 1;
...
/* 	Function: 
	1) Compute the number of training samples for the current tree;
	2) Update $a$ with the samples selected randomly;
	3) For cateroty classification, create an sample order list;
	Variables: 
	- nuse: the number of cases used for training the current tree;
	- ncase: the index of cases/samples used for formulating the current tree; For numerical data, the values in ncase are all '0'
	*/
void modA(int *a, int *nuse, const int nsample, const int mdim, int *cat, const int maxcat, int *ncase, int *jin) 
{
    ...
    *nuse = 0;
	...
	// Update $a$ with the samples selected randomly, ascending order for each variable/dimension;
    for (i = 0; i < mdim; ++i)
	{
		k = 0;
		nt = 0;
...
*/
void Xtranslate(double *x, int mdim, int nrnodes, int nsample, int *bestvar, 
				int *bestsplit, int *bestsplitnext, double *xbestsplit, int *nodestatus, int *cat, int treeSize) 
{
    int i, m;
    for (i = 0; i < treeSize; ++i) 
	{
		if (nodestatus[i] == 1) // splitting node 
		{
			m = bestvar[i] - 1;
			if (cat[m] == 1) 
			{
				xbestsplit[i] = 0.5 * ( x[m + (bestsplit[i] - 1) * mdim] + x[m + (bestsplitnext[i] - 1) * mdim] );
			} 
			else 
			{
